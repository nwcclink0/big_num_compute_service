...
func InitLog() error {
	...
	if err = SetLogLevel(LogAccess, BigNumComputeConf.Log.AccessLevel); err != nil {
		return errors.New("Set access log level error: " + err.Error())
	}

	if err = SetLogLevel(LogError, BigNumComputeConf.Log.ErrorLevel); err != nil {
		return errors.New("Set error log level error: " + err.Error())
	}

	if err = SetLogOut(LogAccess, BigNumComputeConf.Log.AccessLog); err != nil {
		return errors.New("Set access log path error: " + err.Error())
	}

	if err = SetLogOut(LogError, BigNumComputeConf.Log.ErrorLog); err != nil {
...
func LogRequest(uri string, method string, ip string, ContentType string,
	...
	if BigNumComputeConf.Log.Format == "json" {
